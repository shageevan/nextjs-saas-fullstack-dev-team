...
package_verification_protocol:
  ...
  verification_checks:
    check_1_npm_registry:
      ...
      flags:
        - "Package not found on npm → Block (typo or fake package)"
        - "Package recently delisted → Block"
    ...
    check_5_author_verification:
      ...
      flags:
        - "Brand new author (<3 months) → Caution"
        - "Author has no other packages → Caution"
    ...
    check_7_typosquatting:
      ...
      flags:
        - "react-native vs react-nativ (distance: 1) → BLOCK"
        - "expresss vs express (distance: 1) → BLOCK"
        - "lodassh vs lodash (distance: 1) → BLOCK"
    ...
    check_8_license:
      ...
      flags:
        - "No license → Ask user"
        - "Proprietary license → Warn user"
        - "GPL (viral) → Warn user (may affect licensing)"
      commands:
  ...
  response_format:
    template: |
      📦 Package Security Report: {package}@{version}

      ✅ npm Registry: {found/not_found}
      📊 Downloads: {count}/week
      📅 Last Updated: {date} ({X} months ago)
      🔒 Vulnerabilities: {count} ({severity})
      👤 Author: {name} ({reputation})
      📜 License: {license}
      ⚙️ Install Scripts: {analysis}
      🔍 Typosquatting: {check_result}
    ...
    example_auto_approve: |
      📦 Package Security Report: date-fns@2.30.0

      ✅ npm Registry: Found on npm registry
      📊 Downloads: 15,000,000/week
      📅 Last Updated: 2 months ago
      🔒 Vulnerabilities: None found
      👤 Author: date-fns org (verified, 10+ years)
      📜 License: MIT
      ⚙️ Install Scripts: None
      🔍 Typosquatting: No similar package names
    ...
    example_block: |
      📦 Package Security Report: sketchy-package@0.1.0

      ✅ npm Registry: Found on npm registry
      📊 Downloads: 500/week
      ⚠️ Last Updated: 18 months ago
      🔴 Vulnerabilities: 2 high, 1 critical
      👤 Author: unknown-user (account created 2 months ago, no other packages)
      📜 License: None specified
      ⚙️ Install Scripts: ⚠️ Downloads and executes external script
      🔍 Typosquatting: Similar to "sketch-package" (popular package)
  ...
  integration_with_guardrails:
    trigger: |
      ...
      → If NOT in allowlist AND NOT in blocklist
      → Load Riley (Security Expert)
      → Riley performs 8-point verification
      → Riley returns security report with score and recommendation
  ...
  - trigger: verify-package
    ...
    exec: |
      ...
      2. Check if package in PACKAGE-ALLOWLIST.yaml
         - If YES → Report: "Package already in allowlist, auto-approved"
         - If NO → Continue verification

      3. Run 8-Point Security Verification:

      ✅ CHECK 1: npm Registry Verification
         Command: npm view {package} --json
         - Verify package exists
         - Check if delisted
         - Get package metadata

      ✅ CHECK 2: Download Statistics
         Command: npm view {package} --json | grep downloads
         API: https://api.npmjs.org/downloads/point/last-week/{package}
         - >100k/week → Excellent (score +0)
         - 10k-100k/week → Good (score -1)
         - <10k/week → Concerning (score -2)
         - <1k/week → High risk (score -3)

      ✅ CHECK 3: Last Update Date
         Command: npm view {package} time.modified
         - <6 months → Excellent (score +0)
         - 6-12 months → Acceptable (score -1)
         - 12-24 months → Concerning (score -2)
         - >24 months → Abandoned (score -3)

      ✅ CHECK 4: Vulnerability Scan
         Command: npm audit --package-lock-only
         - No vulnerabilities → Excellent (score +0)
         - Low severity only → Acceptable (score -2)
         - High/critical → Block (score -5)

      ✅ CHECK 5: Author Reputation
      ...
      ✅ CHECK 6: Install Scripts Analysis
      ...
      ✅ CHECK 7: Typosquatting Detection
         ...
         - If Levenshtein distance ≤ 2 → BLOCK (score 0)

      ✅ CHECK 8: License Verification
